"""aggregate_job_search fans a profile's per-site queries out to Google's
Custom Search JSON API in parallel and combines them into one result list --
the server-side alternative to paging through GoogleJobSearchResults' free
client-side widget one site at a time.
"""
...
def _settings(**overrides):
    defaults = {"google_custom_search_api_key": "test-key", "google_custom_search_engine_id": "test-cx"}
...
async def test_returns_disabled_with_no_key_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(job_search_aggregation, "get_settings", lambda: _settings(google_custom_search_api_key=None))
