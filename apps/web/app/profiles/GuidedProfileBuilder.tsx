...
const STEPS = [
  { key: 'name', label: 'What should we call this career direction?', help: 'Use a short label such as Quality Leadership or Fractional Engineering.' },
  { key: 'target_titles', label: 'Which titles should Kall look for?', help: 'Confirm close title variants that accurately reflect your experience.' },
  { key: 'industries', label: 'Which industries fit this direction?', help: 'Use short, common industry names separated by commas.' },
  { key: 'include_keywords', label: 'Which strengths should appear in matching roles?', help: 'Only keep skills and specialties your record can support.' },
  { key: 'work_types', label: 'How do you want to work?', help: 'Examples: remote, hybrid, onsite.' },
  { key: 'location', label: 'Where are you willing to work?', help: 'Add countries and states or regions. Leave unknown details unfinished.' },
  { key: 'compensation', label: 'What compensation range should Kall use?', help: 'AI figures are unverified estimates. Replace them with your own decision.' },
] as const;
...
export default function GuidedProfileBuilder({ resumes, onCreated }: { resumes: Resume[]; onCreated: () => Promise<void> }) {
  ...
  const [source, setSource] = useState<'blank' | 'resume'>('blank');
  ...
  function startBlank() { setDraft(EMPTY_DRAFT); setSource('blank'); setStep(0); setMessage('Answer what you know. Kall will keep missing details visible.'); }
