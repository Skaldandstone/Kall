...
The fixture CSP blocks external browser script, connection, frame and form destinations. Next may download public fonts while compiling; wait for the actual fonts before visual review. For opt-in screenshot capture, set `KALL_UI_SCREENSHOTS=1` and run `screenshots.spec.ts` with the same config. Screenshots and viewport/font measurements are written under `test-results`; capture completion is not visual approval.
