...
const statusPath = path.join(iosAssets, 'review-evidence-status.json');
const listingPath = path.join(iosAssets, 'listing.md');
const notesPath = path.join(iosAssets, 'review-notes-draft.md');
const shotListPath = path.join(iosAssets, 'review-evidence-shot-list.md');
...
assert.match(shotList, /Do not substitute responsive web or\s+iOS Simulator media for the physical-device recording/i);
