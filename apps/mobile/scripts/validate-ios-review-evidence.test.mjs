...
function makeEvidence({ captureType = 'physical-device' } = {}) {
  ...
  fs.copyFileSync(
    ...
    path.join(directory, 'plus-subscription-review.png'),
  );
  fs.copyFileSync(
    path.join(mobileRoot, 'store-assets', 'ios', 'screenshots', 'iphone', '2-application-review.png'),
    path.join(directory, 'premium-subscription-review.png'),
  );
...
test('complete fixture produces a hashed evidence manifest', () => {
