...
rotate signing and the App Store Connect API key using
`npx eas-cli@23.2.0 credentials --platform ios`. The current Apple Distribution
certificate, App Store provisioning profile, and `EAS Submit - Kall` API key
are stored in EAS credentials.
