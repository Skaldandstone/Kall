...
The IAM delete statement names the six fixed runtime roles individually. The only
log deletion permission names the stack-managed, 30-day database-admin log group.
The preexisting API and web log groups and the retained expiry recovery log group
...
exact `kall-alpha-postgres` DB ARN in the selected Region. The RDS service
authorization reference requires the source DB resource for this operation; it
does not require a target snapshot resource. The policy therefore grants no
snapshot wildcard and no other RDS create action.
