"""Queue a reminder for every security clearance that just entered its
reminder window.

    python -m kall.jobs.security_clearance_reminders
    python -m kall.jobs.security_clearance_reminders --dry-run
...
def main(argv: list[str] | None = None) -> int:
    ...
    with Session(engine) as session:
        if args.dry_run:
            count = queue_security_clearance_reminders(session, now=datetime.utcnow())
            session.rollback()
            logger.info("Would queue %d reminder(s).", count)
            return 0

        count = queue_security_clearance_reminders(session)
