...
#: `documents.ensure_artifact()`/`ensure_preview()` cache a rendered file by
#: storage key and serve the cached bytes forever once they exist, with no
#: check that the code that produced them still matches what's here now --
#: a resume rendered before a rendering bug fix (the embedded bullet font
#: added below, say) keeps serving the pre-fix bytes indefinitely, since
#: nothing about the cache key or lookup depends on the renderer's code.
#: Bump this whenever a change here or in render_docx/DOCX styling could
#: change already-generated bytes; both cache key builders fold it in, so a
#: bump forces every existing document to re-render once, rather than
#: leaving a fixed bug invisible on every resume generated before the fix.
RENDER_VERSION = 2
