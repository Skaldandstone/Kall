...
def revenuecat_post(path: str, api_key: str) -> dict:
    ...
    if response.status_code >= 400:
        ...
        raise HTTPException(503, f"RevenueCat refused the refund (HTTP {response.status_code})")
    try:
...
def refund_store_subscription(
    ...
    # subscription. Product ids carry the base plan as "product:plan"; the API
    # path takes the product part.
    product = row.product_id.split(":", 1)[0]
    result = revenuecat_post(
        f"/subscribers/{user.clerk_user_id}/subscriptions/{product}/revoke", settings.revenuecat_secret_api_key
    )
