...
4. Public signup: source, Clerk, and the API-tier production deploy are done.
   **The web-image blocker is cleared as of 2 September.** The `pk_test_`-only
   assertion described above no longer exists -- the buildspec now accepts
   `pk_test_*` or `pk_live_*`. Two different things were actually stopping the
   build, both fixed on the `kall-web` CodeBuild project and neither previously
   recorded: `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` was empty, which fails the
   buildspec's own pre_build assertion before Docker starts; and
   `KALL_CLERK_INSTANCE` was absent entirely while the buildspec passes it as a
   `--build-arg`, so it expanded to empty, overrode the Dockerfile's
   `development` default, and dropped `apps/web/Dockerfile`'s case statement
   into its `*)` arm. Both are now set, the key to the `pk_live_` value
   production already serves at `kall.skaldandstone.com` (verified from the live
   page -- a publishable key is public by design, so the build now matches what
   is running rather than introducing an instance). The Dockerfile's own
   mode-aware check was deliberately left alone: it is the correct invariant.
   Still needed before flipping `EnablePublicSignup=true`: the web image build
   and deploy itself, then the controlled real registration and safeguard checks
   in step 8.
5. Public launch legal surface: **written and supplied, not yet deployed.**
   `/terms`, `/support` and a rewritten contact section on `/privacy-policy`
   exist on `feat/public-signup-parameter`, all public in `middleware.ts` and
   linked from the footer. Every business fact reads from
   `apps/web/app/lib/legal.ts`, and `unresolvedLegalFacts()` returns an empty
   array -- operator, Washington formation and governing law, the registered
   agent's Spokane address (filed with WA SoS), privacy@/support@/security@ (all
   three delivering, tested from outside the organization), a 2-business-day
   support reply target, no-refunds-cancel-anytime, and an arbitration clause.
   See `docs/LEGAL.md`. Two things remain: **no lawyer has read it**, section 16
   most of all, and the pages 404 until the web image ships -- same deploy as
   item 4.
