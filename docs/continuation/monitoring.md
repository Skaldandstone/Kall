...
2. Local PostgreSQL 17.11 migration, concurrent transactions and lease recovery now pass on the isolated billing continuation: [evidence](postgres-validation.md). Deployment-specific PostgreSQL/network/TLS verification remains required; local database results are not a cloud runtime result.
