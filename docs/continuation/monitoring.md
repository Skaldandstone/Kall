...
- `opportunities.py`: this lane owns cadence/stale-running changes only. Retain targeting's final stable-job/current-identity canonical dedupe implementation (`02ad005`) when integrating.
...
- UI fixture/config are borrowed from the UI lane's `cd1b1b6` and must be integrated once. This lane contributes only `usability-tests/monitoring.spec.ts` in addition to its two production components.
