...
## Stale canonical-alias follow-up

Confirmed an additional defect: after a posting changes title, company or location, its preserved first-seen key could absorb a different posting that now uses the old identity. Upsert now checks stable `job_id` first. Only when no stable match exists does it inspect canonical-key candidates, and each candidate must also match its linked Job's current normalized company/title/location. Stale candidates are skipped while later valid candidates remain eligible for cross-source deduplication. Existing keys, states, notes, job associations and histories are not rewritten or merged. The existing canonical-key index is non-unique; no migration is required.

Three parameterized regressions failed before the fix, one for each identity field. They now pass and also check that a third source deduplicates into the valid new opportunity, formatting normalization still works, and both histories remain intact. Focused offline validation: `python -m pytest -q tests/test_discovery_refresh.py tests/test_opportunities.py tests/test_scheduled_discovery.py tests/test_digest_profile_eligibility.py --disable-warnings --tb=short` with the same test/default SQLite environment: **36 passed**. Ruff passed for the two changed Python files. The full backend and browser suites were not rerun for this isolated upsert follow-up.

## Visible department evidence follow-up

The integration review found that department/team JSON changed notification fingerprints without contributing functional-area evidence, while provider ID and ordering changes could trigger unnecessary alerts. `services/posting_evidence.py::visible_department_names` now extracts explicit visible names from `team`, `department` and `departments`. It accepts provider string labels and named objects, then normalizes, deduplicates and sorts the names. IDs, ordering, casing, unrelated bookkeeping and office/location objects do not become functional-area evidence.

Matching uses title/description evidence first, preserving existing explanations. When that does not match, it checks each visible department/team name independently and identifies that evidence source in the existing explanation. Names are never concatenated into invented phrases. Body and metadata matches still award only one capped 10-point bonus. Fingerprinting uses the same normalized names; postings without visible names keep their prior fingerprint and scoring behavior.

Validation: **54 focused tests passed** across department evidence, existing matching, opportunity and discovery-refresh suites. **12 tests passed** with an in-memory overlay of this lane's extractor, matcher and fingerprint onto integration commit `a417889`, including the controlled monitoring test. That test verifies a visible name change adds 10 points and one event, ID/order/casing-only changes create no event, removing the name lowers the score, and saved state/notes survive. No files in the integration worktree were modified. Ruff passed for all changed Python files. The standalone monitoring regression skips only when the monitoring module is absent on this older lane base; it executed and passed in the integration overlay. The combined branch should rerun it natively after applying the fix.

Integration ownership: changes in `opportunities.py` are limited to imports and `material_fingerprint`. Replace integration's raw metadata JSON block from `8575362` with the normalized-name block, preserving monitoring's separate upsert/source-association correction. No cloud activity, email, push or merge was performed.
