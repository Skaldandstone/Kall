...
**Deploys are automatic on merge to main.** The `deploy` job in `.github/workflows/ci.yml` runs on every push to main, strictly after all five test jobs pass, decides which image the pushed range actually touched (backend paths → `kall-api-build`, `apps/web/` → `kall-web-build`), and starts the matching CodeBuild build, which builds main's HEAD (pinning the merge commit via `--source-version` is not possible here: CodeBuild fetches a pinned SHA through the GitHub API, which requires stored GitHub credentials even for a public repo, and this setup deliberately stores none - only anonymous branch-HEAD clones work). Each build now deploys itself: the buildspecs live in `ops/codebuild/*.buildspec.yml` (no longer inline in the project config) and end with `ecs update-service --force-new-deployment` plus a `services-stable` wait, so a green build means the rollout completed and passed health checks, and a task that never comes healthy fails the build visibly after ~10 minutes.
