...
| Alerting | SNS topic `kall-alerts` (email subscription to the owner - **pending the confirmation click in that email until acted on**). Alarms into it: `kall-scheduled-job-failures` (metric filter on `/ecs/kall-api` for the job suites' `failed jobs:` line → `Kall/ScheduledJobFailures`), `kall-api-unhealthy-hosts` and `kall-web-unhealthy-hosts` (ALB `UnHealthyHostCount` ≥ 1 for 10 min, with OK notifications). Deploy failures are covered separately by GitHub's own failed-workflow emails |
| Log retention | All four log groups (`/ecs/kall-api`, `/ecs/kall-web`, both CodeBuild groups) capped at 30 days - they defaulted to never-expire |
